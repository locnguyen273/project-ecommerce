...
module.exports.index = async (req, res) => {
  ...
  if (!keyWordSearch) {
    ...
    var newData = coupon.filter((value) => {
      return (
        value.code.toUpperCase().indexOf(keyWordSearch.toUpperCase()) !== -1 ||
        value.promotion.toUpperCase().indexOf(keyWordSearch.toUpperCase()) !==
          -1
      );
    });
...
module.exports.checking = async (req, res) => {
  ...
  const checkCoupon = await Order.findOne({
    id_user: id_user,
