...
module.exports.send_mail = async (req, res) => {
  const carts = await Detail_Order.find({
  ...
  for (let i = 0; i < carts.length; i++) {
    htmlContent +=
      ...
      carts[i].id_product.name_product +
      "</td>" +
      '<td style="border: 1px solid black; font-size: 1.2rem; text-align: center;"><img src="' +
      carts[i].id_product.image +
      '" width="80" height="80"></td>' +
      '<td style="border: 1px solid black; font-size: 1.2rem; text-align: center;">' +
      carts[i].id_product.price_product +
      "$</td>" +
      '<td style="border: 1px solid black; font-size: 1.2rem; text-align: center;">' +
      carts[i].count +
      "</td>" +
      ...
      "</td>" +
      '<td style="border: 1px solid black; font-size: 1.2rem; text-align: center;">' +
      parseInt(carts[i].id_product.price_product) * parseInt(carts[i].count) +
      "$</td>" +
      "<tr>";
  }
...
module.exports.get_detail = async (req, res) => {
  ...
  const order = await Order.findOne({ _id: id_order }).populate([
    "id_user",
    "id_note",
    "id_payment",
  ]);
