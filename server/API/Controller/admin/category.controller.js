...
module.exports.index = async (req, res) => {
  ...
  if (!keyWordSearch) {
    ...
    var newData = categories.filter((value) => {
      return (
        value.category.toUpperCase().indexOf(keyWordSearch.toUpperCase()) !==
          -1 ||
...
module.exports.create = async (req, res) => {
  ...
  if (categoryFilter.length > 0) {
    ...
    req.query.name = req.query.name.toLowerCase().replace(/^.|\s\S/g, (a) => {
...
module.exports.detailProduct = async (req, res) => {
  ...
  let products = await Product.find({ id_category: category._id }).populate(
    "id_category"
  );
  ...
  if (!keyWordSearch) {
    ...
    var newData = products.filter((value) => {
      return (
        value.name_product
          .toUpperCase()
          .indexOf(keyWordSearch.toUpperCase()) !== -1 ||
        value.price_product
          .toUpperCase()
          .indexOf(keyWordSearch.toUpperCase()) !== -1 ||
        value.id.toUpperCase().indexOf(keyWordSearch.toUpperCase()) !== -1
...
module.exports.update = async (req, res) => {
  ...
  const categoryFilter = category.filter((c) => {
    return (
      c.category.toUpperCase() === req.query.name.toUpperCase().trim() &&
      c.id !== req.query.id
    );
  });
