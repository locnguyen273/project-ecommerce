...
module.exports.index = async (req, res) => {
  ...
  if (req.query.permission) {
    users = await User.find({ id_permission: req.query.permission }).populate(
      "id_permission"
    );
  } else {
  ...
  if (!keyWordSearch) {
    ...
    var newData = users.filter((value) => {
      return (
        value.fullname.toUpperCase().indexOf(keyWordSearch.toUpperCase()) !==
          -1 ||
...
module.exports.create = async (req, res) => {
  ...
  const userFilter = user.filter((c) => {
    return (
      c.email === req.query.email.trim() ||
      c.username === req.query.username.trim()
    );
  });
